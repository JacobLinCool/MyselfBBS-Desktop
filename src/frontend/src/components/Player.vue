...
watch(props, () => {
    const reuse = prev_vid.value === props.vid;
    let preloaded = false;
    last_update = 0;
    ...
    if (!reuse) {
        ...
        console.log(`created video element`, elm);
    }

    if (props.vid && props.ep) {
        if (!video && elm) {
            video = videojs(elm, {
                controls: true,
                autoplay: true,
            ...
            console.log(`created video player`, video);
        } else if (video) {
            video.src(`${config.backend}/${props.vid}/${props.ep}/index.m3u8`);
        }
    ...
    prev_vid.value = props.vid;
    prev_ep.value = props.ep;
});
